...
public class ClinicController {
    ...
    @PostMapping
    public ResponseEntity<Void> create(@RequestBody CreateClinicDTO clinic) {
        clinicService.create(clinic);
        return ResponseEntity.noContent().build();
    }
    @PutMapping("/{id}")
    public ResponseEntity<Void> update(@RequestBody UpdateClinicDTO clinic, @PathVariable UUID id) {
